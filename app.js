...
function filterEventsByArea(neighbourhood) {
    ...
    document.getElementById("eventsPanel").scrollIntoView({
        behavior: "smooth"
    });
}
...
async function addEventServer(eventData) {
    try {
        ...
        const response = await fetch("/server/addevent", {
            method: "POST",
            body: formData
        });
...
function renderSpotifyAudienceMap() {
    ...
    for (const item of spotifyAudience) {
        ...
        if (!coords) {
            console.warn("Unknown city:", item.city);
            continue;
        }
    ...
    if (audienceMarkers.length > 0) {
        map.fitBounds(L.featureGroup(audienceMarkers).getBounds(), {
            padding: [40, 40]
        });
    }
}
