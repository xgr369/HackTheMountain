...
function addInterest(eventId) {
  ...
  if (event) { event.interested++; saveEvents(); renderEvents(activeFilter); }
}
...
async function createEvent() {
  ...
  const btn = document.getElementById("createEventBtn") || document.querySelector(".create-panel .create-btn");
...
function scoreNeighbourhood(n, inputs) {
  ...
  // For ticketed venues (concert_hall, community) foot traffic matters less
...
function buildReasons(n, meta, inputs) {
  ...
  switch (inputs.performanceType) {
    ...
    case "park":
      ...
      break;
    case "bar_cafe":
      if (n.venues.length > 0) reasons.push(`Known live music venues: ${n.venues.slice(0, 2).join(", ")}`);
      else reasons.push("Bar and café scene active — reach out to local spots for a slot");
      break;
    case "concert_hall":
      if (meta.venueQuality === 3) reasons.push(`Premier stages available: ${n.venues.slice(0, 2).join(", ")}`);
      else if (meta.venueQuality === 2) reasons.push(`Mid-size venues: ${n.venues.join(", ")}`);
...
// ─────────────────────────────────────────────
//  Fan / Creator Login + Spotify Audience Demo
// ─────────────────────────────────────────────
...
function loginAs(type) {
  currentUserType = currentUserType === type ? null : type;
  ...
  if (creatorPanel) creatorPanel.style.display = currentUserType === "creator" ? "block" : "none";
  if (fanPanel) fanPanel.style.display = currentUserType === "fan" ? "block" : "none";

  if (fanBtn) fanBtn.classList.toggle("active", currentUserType === "fan");
  if (creatorBtn) creatorBtn.classList.toggle("active", currentUserType === "creator");
}
...
function connectSpotifyAudience() {
  ...
  if (!rawText) {
    result.innerHTML = "Paste city data first, example: Montreal, 4200";
    return;
  }

  spotifyAudience = rawText
    .split("\n")
    .map(line => {
  ...
  if (spotifyAudience.length === 0) {
    result.innerHTML = "No valid Spotify audience data found.";
    return;
  }
...
function renderSpotifyAudienceMap() {
  audienceMarkers.forEach(marker => map.removeLayer(marker));
  audienceMarkers = [];
  ...
  const maxListeners = Math.max(...spotifyAudience.map(item => item.listeners));

  for (const item of spotifyAudience) {
    const coords = cityCoords[item.city];
    if (!coords) {
      console.warn("Unknown city. Add coordinates for:", item.city);
      continue;
    }
    ...
    const circle = L.circle(coords, {
      ...
      <div class="event-popup">
        ...
        <small>Approximate city-level fan demand, not exact fan locations.</small>
      </div>
    `);
  ...
  if (audienceMarkers.length > 0) {
    const group = L.featureGroup(audienceMarkers);
...
const fanLoginBtn = document.getElementById("fanLoginBtn");
if (fanLoginBtn) fanLoginBtn.addEventListener("click", () => loginAs("fan"));

const creatorLoginBtn = document.getElementById("creatorLoginBtn");
if (creatorLoginBtn) creatorLoginBtn.addEventListener("click", () => loginAs("creator"));

const spotifyImportBtn = document.getElementById("spotifyImportBtn");
if (spotifyImportBtn) spotifyImportBtn.addEventListener("click", connectSpotifyAudience);

const saveFanBtn = document.getElementById("saveFanBtn");
if (saveFanBtn) saveFanBtn.addEventListener("click", saveFanProfile);
